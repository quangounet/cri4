...
for i in xrange(0,len(t)):
    time.append(Decimal(t[i])/120)

newx=butter_lowpass_filter(x,lowcut,fs,order=2)
newy=butter_lowpass_filter(y,lowcut,fs,order=2)
plt.plot(time,newx,label='Filtered x signal')
plt.plot(time,newy,label='Filtered y signal')
plt.plot(time,x,label='Noisy x signal')
plt.plot(time,y,label='Noisy y signal')
plt.legend(loc='lower right')

plt.figure(7)
plt.plot(x,y,newx,newy)
